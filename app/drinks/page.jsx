...
const menudrink = () => {
  ...
  <Link href={`drinkdetails/${drink.id}`}>
    <div className='rounded-[30px]  relative  border-[#EFEEEE] border-[1.5px] bg-[#FFF] h-[230px] w-[200px] md:h-[370px] md:w-[320px]  flex flex-col md:justify-between' key={drink.id} id={drink.id}>
      <div className='relative bottom-[50px] flex justify-center  w-full h-[200px] '>
        <Image src={drink.img} width={270}  height={270}
         alt='pictures of food from different sites and a little from ris'
         priority
          className='rounded-full  border-[13px]  flex-shrink-0 w-[140px] h-[140px] md:h-[230px] md:w-[200px] ' placeholder='blur' blurDataURL={drink.blurImage}/>
        </div>
       <div className='flex flex-col  mb-10 '>
        <p className=' capitalize opacity-60 font-semibold  text-center text-[15px] md:text-[22px]'>{drink.name}</p>
        <p className='text-center text-[#FFC83A] font-semibold text-[12px] md:text-[20px]'>{drink.price}</p>
      </div>
     </div>
    </Link>
  ))
